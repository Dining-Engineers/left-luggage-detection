...
Notes about the coordinate systems. There are three coordinate systems to worry about.
 ...
 1. Kinect depth image: (u, v, depth) u and v are image coordinates,
 ...
 2. Kinect rgb image: (u, v) u and v are image coordinates

    (0,0) is the top left corner
    (640,480) is the bottom right corner
                              
 3. XYZ world coordinates: (x, y, z)
...
def depth2xyzuv(depth, u=None, v=None):
    ...
    :param depth: comes directly from the kinect
    :param u: image coordinates, same size as depth (default is the original image)
    :param v: image coordinates, same size as depth (default is the original image)
...
def xyz_matrix():
    """
    Returns a matrix you can use to project U,V  coordinates (in meters) into
    XYZ coordinates in the kinect RGB image
